...
This repository is the implementation for **SoundBox** (`com.cjhackeryt.soundbox`), an out-of-process Windows soundboard plugin for **Macro Deck 3**.
...
```
SoundBox.sln                 solution file referencing plugin and test projects
SoundBox.csproj              the plugin project targeting net10.0 (win-x64)
...
```bash
macrodeck-plugin test --artifact ./artifacts/com.cjhackeryt.soundbox-0.1.3.macroDeckPlugin --report markdown --output conformance.md
```
...
macrodeck-plugin inspect --artifact ./artifacts/com.cjhackeryt.soundbox-0.1.3.macroDeckPlugin
```
